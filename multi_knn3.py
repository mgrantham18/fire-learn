...
for row in fires:                                   #Fill list with the data about fires
    if counter < limit:
        if(row[2] == ""):                           #if end DOY missing assume it is the same as the start
            X.append([int(row[1]), int(row[1]), float(row[3]), float(row[4]), float(row[5]), int(row[6]), int(row[7])])
        else:
            X.append([int(row[1]), int(row[2]), float(row[3]), float(row[4]), float(row[5]), int(row[6]), int(row[7])])
    else:
        break
    counter+=1
...
for row in fires:                                   #skip to the last 100000 fires and test the model
    if(counter > len(fires)-100000): #60034
        if(counter%100 == 0):
            print("Currently tested " + str(counter-(len(fires)-10000)) + " rows.")
        prediction = []
        if(row[2] != ""):
            for knn in knn_class:
                prediction.append(knn.predict([[int(row[1]), int(row[2]), float(row[3]), float(row[4]), float(row[5]), int(row[6]), int(row[7])]])[0])
        else:
            for knn in knn_class:
                prediction.append(knn.predict([[int(row[1]), int(row[1]), float(row[3]), float(row[4]), float(row[5]), int(row[6]), int(row[7])]])[0])
        #print(prediction)
