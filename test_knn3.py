...
while number_of_neighbors < 1000:                       #test knn for k all ints from 1 to 1000
    ...
    for row in fires:                                   #skip to the last 100000 fires and test the model
        if(counter > len(fires)-100000):
            prediction = 0
            if(row[2] != ""):
                prediction = knn.predict([[int(row[1]), int(row[2]), float(row[3]), float(row[4]), float(row[5]), int(row[6]), int(row[7])]])
            else:
                prediction = knn.predict([[int(row[1]), int(row[1]), float(row[3]), float(row[4]), float(row[5]), int(row[6]), int(row[7])]])
    ...
    file = open('results7.csv', 'a')                     #insert the results into a csv
