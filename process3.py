...
reporters = []
for row in init_fires:                                 #create list of the names of the causes
    if row[6] not in reporters:
        reporters.append(row[6])
        print(row)
...
for row in init_fires:                                 #convert targets to ints based on index in name list
    ...
    new_row.append(reporters.index(row[6]))
    new_row.append(row[7])
    new_row.append(owners.index(row[8]))
    fires.append(new_row)

#print(init_fires)
#print(owners)
#print(fires)
f = open('processed3.csv', 'a')
for row in fires:
    line = str(row[0]) + "," + str(row[1]) + "," + str(row[2]) + "," + str(row[3]) + "," + str(row[4]) + "," + str(row[5]) + "," + str(row[6]) + "," + str(row[7]) + "," + str(row[8]) +"\n"
    f.write(str(line))
f.close()
