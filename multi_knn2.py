    ...
    if(counter > len(fires)-100000): #60034
        ...
        if(row[2] != ""):
            for knn in knn_class:
                prediction.append(knn.predict([[int(row[1]), int(row[2]), float(row[3]), float(row[4]), float(row[5]), int(row[6])]])[0])
        else:
            for knn in knn_class:
                prediction.append(knn.predict([[int(row[1]), int(row[1]), float(row[3]), float(row[4]), float(row[5]), int(row[6])]])[0])
        #print(prediction)
