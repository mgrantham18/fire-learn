#MODEL USING START_DOY, END_DOY, SIZE, LAT, LONG, AND OWNER
...
while number_of_neighbors < 1000:                       #test knn for k all ints from 1 to 1000
    ...
    for row in fires:                                   #Fill list with the data about fires
        if counter < limit:
            if(row[2] == ""):                           #if end DOY missing assume it is the same as the start
                X.append([int(row[1]), int(row[1]), float(row[3]), float(row[4]), float(row[5]), int(row[6])])
            else:
                X.append([int(row[1]), int(row[2]), float(row[3]), float(row[4]), float(row[5]), int(row[6])])
        else:
            break
        counter+=1
    ...
    for row in fires:                                   #skip to the last 100000 fires and test the model
        if(counter > len(fires)-100000):
            prediction = 0
            if(row[2] != ""):
                prediction = knn.predict([[int(row[1]), int(row[2]), float(row[3]), float(row[4]), float(row[5]), int(row[6])]])
            else:
                prediction = knn.predict([[int(row[1]), int(row[1]), float(row[3]), float(row[4]), float(row[5]), int(row[6])]])
    ...
    file = open('results5.csv', 'a')                     #insert the results into a csv
